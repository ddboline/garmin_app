...
class GarminCache(object):
    ...
    def get_cached_object_in_file(self, fname='', garmin_class=None):
        if not os.path.exists(fname) or not garmin_class:
            return False
        else:
            return True

    def write_cached_object_to_file(self, inobj=None, outfile=None):
        if not inobj or not os.path.exists(outfile):
            return False
        else:
            return True
