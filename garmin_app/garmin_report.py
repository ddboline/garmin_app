...
class GarminReport(object):
    ...
    def summary_report(self, summary_list, copy_to_public_html=True,
        ...
        for gfile in summary_list:
            ...
            week_index = cur_date.isocalendar()[0]*100 +\
                         cur_date.isocalendar()[1]
            sport = gfile.sport
    ...
    def week_average_report_txt(gsum, sport=None, number_days=0,
        ...
        if sport == 'total':
            ...
            retval.append(
                ...
                % (gsum.total_distance/METERS_PER_MILE/number_of_weeks),
                '%i cal' % (gsum.total_calories/number_of_weeks)))

        if sport == 'running' or sport == 'walking':
            retval.append(
                ' %10s \t' % ('%s / mi' % print_h_m_s(gsum.total_duration
                / (gsum.total_distance / METERS_PER_MILE), False)))
            retval.append(
                ' %10s \t' % ('%s / km' % print_h_m_s(gsum.total_duration
                / (gsum.total_distance / 1000.), False)))
        ...
        if gsum.total_hr_dur > 0 and sport != 'total':
            ...
            retval.append(' %7s %2s' % (' ', ' '))
        retval.append(
            '%16s' % ('%.1f / %i days' % (float(number_days)
            / number_of_weeks, 7)))
        return ' '.join(retval)
    ...
    def month_average_report_txt(gsum, sport=None, number_of_months=0):
        ...
        if sport == 'total':
            retval.append(
                '%17s %10s \t %10s \t %10s \t' % ('average / month', sport,
                ' ', '%i cal' % (gsum.total_calories / number_of_months)))
        else:
            retval.append(
                '%17s %10s \t %10s \t %10s \t' % ('average / month', sport,
                '%4.2f mi' % (gsum.total_distance / METERS_PER_MILE /
                number_of_months), '%i cal' % (gsum.total_calories /
                number_of_months)))
        if sport == 'running' or sport == 'walking':
            retval.append(
                ' %10s \t' % ('%s / mi' % print_h_m_s(gsum.total_duration
    ...
    for dis, tim, hrt in split_vector:
        retval.append('%i %s \t %s \t %s / mi \t %s / km \t %s \t %i bpm avg'
            % (dis,
               ...
               print_h_m_s(tim),
               print_h_m_s(
                   tim/(split_distance_in_meters/METERS_PER_MILE), False),
               print_h_m_s(tim/(split_distance_in_meters/1000.), False),
               print_h_m_s(
                   tim/(split_distance_in_meters/METERS_PER_MILE)
...
def get_lap_html(glap, sport):
    ...
    if glap.lap_distance > 0:
        values.extend([
            '%s / mi' % print_h_m_s(glap.lap_duration / (glap.lap_distance
            / METERS_PER_MILE), False),
            '%s / km' % print_h_m_s(glap.lap_duration / (glap.lap_distance
            / 1000.), False),])
    if glap.lap_avg_hr:
