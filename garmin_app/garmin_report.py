...
class GarminReport(object):
    ...
    def summary_report(self, summary_list, options, copy_to_public_html=True):
        ...
        month_set = list(set((x.begin_datetime.year*100 +
                              x.begin_datetime.month) for x in summary_list))
        week_set = list(set((x.begin_datetime.isocalendar()[0]*100 +
                             x.begin_datetime.isocalendar()[1])
        ...
        year_sport_summary = {s: {y: GarminSummary() for y in year_set}
                              for s in sport_set}
        month_sport_summary = {s: {m: GarminSummary() for m in month_set}
                               for s in sport_set}
        day_sport_summary = {s: {d: GarminSummary() for d in day_set}
                             for s in sport_set}
        week_sport_summary = {s: {w: GarminSummary() for w in week_set}
                              for s in sport_set}

        total_sport_day_set = {s: set() for s in sport_set}
        year_sport_day_set = {s: {y: set() for y in year_set}
                              for s in sport_set}
        month_sport_day_set = {s: {m: set() for m in month_set}
                               for s in sport_set}
        day_sport_set = {s: set() for s in sport_set}
        week_sport_day_set = {s: {w: set() for w in week_set}
                              for s in sport_set}
            ...
            for sport in SPORT_TYPES:
                ...
                for gfile in summary_list:
                    ...
                    if gfile.sport != sport:
                        continue
                    retval.append(self.day_summary_report_txt(gfile, sport,
                                                              cur_date))
        ...
        if do_day:
            for sport in SPORT_TYPES:
                ...
                for cur_date in day_set:
                    if cur_date not in day_sport_set[sport]:
                        continue
                    retval.append(
                        self.day_summary_report_txt(
                            day_sport_summary[sport][cur_date], sport,
                            cur_date))
                    cmd_args.append(
                        '%04d-%02d-%02d file %s' % (cur_date.year,
                                                    cur_date.month,
                                                    cur_date.day,
                                                    sport))
                retval.append('')
                if not do_sport and do_average:
                    retval.append(
                        self.day_average_report_txt(
                            total_sport_summary[sport], sport,
            ...
            if not do_sport:
                for cur_date in day_set:
                    retval.append(
                        self.day_summary_report_txt(day_summary[cur_date],
                                                    'total', cur_date))
                    cmd_args.append('%04d-%02d-%02d file %s' % (cur_date.year,
                                                                cur_date.month,
                                                                cur_date.day,
                ...
                retval.append('')
            if not do_sport and do_average:
                retval.append(self.day_average_report_txt(total_summary,
                                                          'total',
                                                          len(day_set)))
                cmd_args.append('')
                retval.append('')
        if do_week:
            for sport in SPORT_TYPES:
                ...
                for yearweek in week_set:
                    ...
                    retval.append(
                        self.week_summary_report_txt(
                            week_sport_summary[sport][yearweek], sport,
                            isoyear, isoweek,
                ...
                if not do_sport and do_average:
                    retval.append(
                        self.week_average_report_txt(
                            total_sport_summary[sport], sport=sport,
            ...
            for yearweek in week_set:
                ...
                if not do_sport:
                    retval.append(
                        self.week_summary_report_txt(
                            week_summary[yearweek], 'total', isoyear, isoweek,
                            len(week_day_set[yearweek])))
                    cmd_args.append('')
            if not do_sport and do_average:
                retval.append('')
                retval.append(
                    self.week_average_report_txt(
                        total_summary, sport='total',
        ...
        if do_month:
            for sport in SPORT_TYPES:
                ...
                for yearmonth in month_set:
                    ...
                    if len(month_sport_day_set[sport][yearmonth]) == 0:
                        continue
                    retval.append(
                        self.month_summary_report_txt(
                            month_sport_summary[sport][yearmonth], sport,
                            year, month,
            ...
            for yearmonth in month_set:
                ...
                if not do_sport:
                    retval.append(
                        self.month_summary_report_txt(
                            month_summary[yearmonth], 'total', year, month,
                            len(month_day_set[yearmonth])))
            ...
            if not do_sport and do_average:
                retval.append(
                    self.month_average_report_txt(
                        total_summary, sport='total',
                        number_of_months=len(month_set)))
                cmd_args.append('')
                retval.append('')
        if do_year:
            for sport in SPORT_TYPES:
                ...
                for year in year_set:
                    if len(year_sport_day_set[sport][year]) == 0:
                        continue
                    retval.append(
                        self.year_summary_report_txt(
                            year_sport_summary[sport][year], sport, year,
        ...
        if not do_sport and do_average:
            ...
            retval.append(
                self.day_average_report_txt(
                    total_summary, 'total', len(day_set)))
            cmd_args.append('')
            retval.append('')
        for sport in SPORT_TYPES:
            if sport not in sport_set:
        ...
        if not do_sport:
            ...
            retval.append(
                self.total_summary_report_txt(
                    total_summary, 'total', len(day_set), total_days))
            cmd_args.append('')
            retval.append('')
    ...
    def file_report_txt(gfile):
        ...
        if gfile.total_distance > 0:
            min_mile = (gfile.total_duration / 60.) / (gfile.total_distance
                                                       / METERS_PER_MILE)
        ...
        if sum_time > 0:
            ...
            if len(hr_vals) > 0:
                retval.append(
                    'Heart Rate %2.2f avg %2.2f max' % (avg_hr, max(hr_vals)))
        ...
        if len(alt_vals) > 0:
            retval.append(
                'max altitude diff: %.2f m' % (max(alt_vals) - min(alt_vals)))
    ...
    def file_report_html(self, gfile, options=None, use_time=False,
        ...
        lon_vals = []
        graphs = []
        mile_split_vals = get_splits(gfile, METERS_PER_MILE,
                                     do_heart_rate=False)
        if mile_split_vals:
                ...
                with open('%s/templates/MAP_TEMPLATE.html' %
                    ...
                    for line in infile:
                        if 'SPORTTITLEDATE' in line:
                            ...
                            _tmp = get_html_splits(
                                gfile, split_distance_in_meters=5000.,
                                label='km')
                            if _tmp is not None:
                                htmlfile.write('<br><br>%s\n' % _tmp)
        ...
        if copy_to_public_html and (os.path.exists('%s/html' % cache_dir)
                                    and os.path.exists(
                                        '%s/public_html/garmin' % HOMEDIR)):
    ...
    def total_summary_report_txt(gsum, sport=None, number_days=0,
        ...
        if sport == 'running' or sport == 'walking':
            ...
            retval.append(
                ' %10s \t' % (
                    '%s / km' % print_h_m_s(
                        gsum.total_duration / (gsum.total_distance / 1000.),
                        False)))
        elif sport == 'biking':
            retval.append(
                ' %10s \t' % (
        ...
        if number_days > 0 and total_days > 0:
            retval.append('%16s' % ('%i / %i days' % (number_days,
                                                      total_days)))
        return ' '.join(retval)
        ...
        if sport == 'running' or sport == 'walking':
            if gsum.total_distance > 0:
                ...
                retval.append(
                    ...
                    % ('%10s %02i %3s' % (cur_date, week, weekdayname), sport,
                       '%.2f mi' % (gsum.total_distance/METERS_PER_MILE),
                       '%i cal' % gsum.total_calories, '         / mi',
                       '         / km', print_h_m_s(gsum.total_duration)))
        elif sport == 'biking':
            retval.append(
    ...
    def day_average_report_txt(gsum, sport=None, number_days=0):
        ...
        if sport == 'running' or sport == 'walking':
            retval.append(
                '%17s %10s \t %10s \t %10s \t %10s \t %10s \t %10s'
                % ('average / day', sport, '%.2f mi' % (gsum.total_distance
                                                        / METERS_PER_MILE
                                                        / number_days),
                   ...
                   '%s / km' % print_h_m_s(gsum.total_duration
                                           / (gsum.total_distance / 1000.),
                                           False),
                   print_h_m_s(gsum.total_duration/number_days)))
        elif sport == 'biking':
            retval.append(
                '%17s %10s \t %10s \t %10s \t %10s \t %10s'
                % ('average / day', sport, '%.2f mi'
                   % (gsum.total_distance/METERS_PER_MILE/number_days),
                   '%i cal' % (gsum.total_calories/number_days),
                   '%.2f mph' % ((gsum.total_distance/METERS_PER_MILE)
                                 / (gsum.total_duration / 60. / 60.)),
                   print_h_m_s(gsum.total_duration/number_days)))
    ...
    def week_summary_report_txt(gsum, sport=None, isoyear=None, isoweek=None,
        ...
        if sport == 'total':
            retval.append('%17s %10s \t %10s \t %10s \t'
                          % ('%i week %02i' % (isoyear, isoweek), sport, ' ',
                             '%i cal' % gsum.total_calories))
        else:
            retval.append('%17s %10s \t %10s \t %10s \t'
                          % ('%i week %02i' % (isoyear, isoweek), sport,
                             '%4.2f mi' % (gsum.total_distance
                                           / METERS_PER_MILE),
                             '%i cal' % gsum.total_calories))

        if sport == 'running' or sport == 'walking':
            retval.append(' %10s \t' % ('%s / mi' % print_h_m_s(
                gsum.total_duration / (gsum.total_distance / METERS_PER_MILE),
                False)))
            retval.append(' %10s \t' % ('%s / km' % print_h_m_s(
                gsum.total_duration / (gsum.total_distance / 1000.), False)))
        elif sport == 'biking':
            retval.append(' %10s \t' % ('%.2f mph' % ((gsum.total_distance
                                                       / METERS_PER_MILE)
                                                      / (gsum.total_duration
                                                         / 60. / 60.))))
        else:
        ...
        retval.append(' %10s \t' % (print_h_m_s(gsum.total_duration)))
        if gsum.total_hr_dur > 0 and sport != 'total':
            retval.append(' %7s %2s' % ('%i bpm' % (gsum.total_hr_dur
                                                    / gsum.total_hr_dis), ' '))
        else:
            retval.append(' %7s %2s' % (' ', ' '))
        retval.append(
    ...
    def week_average_report_txt(gsum, sport=None, number_days=0,
        ...
        retval = []
        if sport == 'total':
            retval.append('%17s %10s \t %10s \t %10s \t' % ('avg / %3s weeks'
                          % number_of_weeks, sport, ' ', '%i cal'
                          % (gsum.total_calories/number_of_weeks)))
        else:
            retval.append('%17s %10s \t %10s \t %10s \t' % ('avg / %3s weeks'
                          % number_of_weeks, sport, '%4.2f mi'
                          % (gsum.total_distance / METERS_PER_MILE
                             / number_of_weeks), '%i cal' % (
                                                 gsum.total_calories
                                                 / number_of_weeks)))

        if sport == 'running' or sport == 'walking':
            retval.append(' %10s \t' % ('%s / mi' % print_h_m_s(
                          gsum.total_duration / (gsum.total_distance
                                                 / METERS_PER_MILE), False)))
            retval.append(' %10s \t' % ('%s / km' % print_h_m_s(
                          gsum.total_duration / (gsum.total_distance / 1000.),
                          False)))
            ...
            retval.append(' %10s \t' % ' ')
        retval.append(
            ' %10s \t' % (print_h_m_s(gsum.total_duration / number_of_weeks)))
        if gsum.total_hr_dur > 0 and sport != 'total':
            retval.append(' %7s %2s' % ('%i bpm' % (gsum.total_hr_dur
                                                    / gsum.total_hr_dis), ' '))
        else:
            retval.append(' %7s %2s' % (' ', ' '))
        retval.append('%16s' % ('%.1f / %i days' % (float(number_days)
                                                    / number_of_weeks, 7)))
        return ' '.join(retval)
    ...
    def month_summary_report_txt(gsum, sport=None, year=None, month=None,
        ...
        if sport == 'total':
            ...
            retval.append('%17s %10s \t %10s \t %10s \t' % ('%i %s'
                          % (year, MONTH_NAMES[month-1]), sport, '%4.2f mi'
                          % (gsum.total_distance/METERS_PER_MILE), '%i cal'
                          % gsum.total_calories))
        if sport == 'running' or sport == 'walking':
            retval.append(' %10s \t' % ('%s / mi' % print_h_m_s(
                          gsum.total_duration / (gsum.total_distance
            ...
            retval.append(' %10s \t' % ('%.2f mph' % ((gsum.total_distance
                          / METERS_PER_MILE) / (gsum.total_duration / 60.
                                                / 60.))))
        else:
            retval.append(' %10s \t' % ' ')
        retval.append(
            ' %10s \t' % (print_h_m_s(gsum.total_duration)))
        if gsum.total_hr_dur > 0:
            retval.append(' %7s %2s' % ('%i bpm' % (gsum.total_hr_dur
                          / gsum.total_hr_dis), ' '))
        else:
    ...
    def month_average_report_txt(gsum, sport=None, number_of_months=0):
        ...
        retval = []
        if sport == 'total':
            retval.append('%17s %10s \t %10s \t %10s \t' % ('average / month',
                          sport, ' ', '%i cal' % (gsum.total_calories
                                                  / number_of_months)))
        else:
            retval.append('%17s %10s \t %10s \t %10s \t' % ('average / month',
                          sport, '%4.2f mi' % (gsum.total_distance
                                               / METERS_PER_MILE
                                               / number_of_months),
        ...
        if sport == 'running' or sport == 'walking':
            ...
            retval.append(' %10s \t' % ' ')
        retval.append(
            ' %10s \t' % (print_h_m_s(gsum.total_duration/number_of_months)))
        if gsum.total_hr_dur > 0:
            retval.append(' %7s %2s' % ('%i bpm' % (gsum.total_hr_dur
                          / gsum.total_hr_dis), ' '))
        else:
    ...
    def year_summary_report_txt(gsum, sport=None, year=None, number_in_year=0):
        ...
        if gsum.total_hr_dur > 0:
            ...
            retval.append(' %7s %2s' % (' ', ' '))
        retval.append(
            '%16s' % ('%i / %i days' % (number_in_year, total_days)))
        return ' '.join(retval)
...
def print_lap_string(glap, sport):
    ...
    if sport == 'running':
        if glap.lap_distance > 0:
            outstr.extend([print_h_m_s(glap.lap_duration
                                       / (glap.lap_distance
                                          / METERS_PER_MILE), False), '/ mi ',
...
def get_splits(gfile, split_distance_in_meters=METERS_PER_MILE, label='mi',
    ...
    for point in gfile.points:
        ...
        if nmiles > 0:
            ...
            cur_split_time = (last_point_time + (cur_point_time -
                              last_point_time) / (cur_point_me -
                              last_point_me) * (cur_split_me - last_point_me))
    ...
    for dis, tim, hrt in split_vector:
        retval.append('%i %s \t %s \t %s / mi \t %s / km \t %s \t %i bpm avg'
                      % (dis, label, print_h_m_s(tim),
                         print_h_m_s(tim / (split_distance_in_meters
                                     / METERS_PER_MILE), False),
                         print_h_m_s(tim / (split_distance_in_meters / 1000.),
                                     False),
                         print_h_m_s(tim / (split_distance_in_meters
                                     / METERS_PER_MILE)
...
def get_file_html(gfile):
    ...
    retval.append(
        '<thead><tr style="text-align: center;"><th>Start Time</th>'
        '<th>Sport</th></tr></thead>')
    retval.append(
        '<tbody><tr style="text-align: center;"><td>%s</td><td>%s</td>'
        % (print_date_string(gfile.begin_datetime), gfile.sport)
        + '</tr></tbody>')
    retval.append('</table><br>')

    labels = ['Sport', 'Lap', 'Distance', 'Duration', 'Calories', 'Time',
              'Pace / mi', 'Pace / km', 'Heart Rate']
    retval.append('<table border="1" class="dataframe">')
...
def get_lap_html(glap, sport):
    ...
    if glap.lap_distance > 0:
        values.extend(['%s / mi' % print_h_m_s(glap.lap_duration
                       / (glap.lap_distance / METERS_PER_MILE), False),
                       '%s / km' % print_h_m_s(glap.lap_duration
                                               / (glap.lap_distance / 1000.),
                                               False)])
    if glap.lap_avg_hr:
...
def get_html_splits(gfile, split_distance_in_meters=METERS_PER_MILE,
    ...
    labels = ['Split', 'Time', 'Pace / mi', 'Pace / km', 'Marathon Time',
              'Heart Rate']
    values = []

    split_vector = get_splits(gfile, split_distance_in_meters, label)
    for dis, tim, hrt in split_vector:
        tmp_vector = ['%i %s' % (dis, label),
                      print_h_m_s(tim),
                      print_h_m_s(tim / (split_distance_in_meters
                                  / METERS_PER_MILE), False),
                      print_h_m_s(tim / (split_distance_in_meters / 1000.),
